...
| Save fails with "data contract violation: …" | Write gate | `wardrobeItemsUpsertRowViolations` app.js | The gate rejected the built upsert row (DATA-CONTRACT.md → Write gate). The message names the offending field. Usually the caller passed junk (stray metadata key, malformed date) — fix the caller/data, don't loosen the gate. |
